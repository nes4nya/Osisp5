...
#include "../ProcListener.Lib/LCWCommon.h"
#include "../ProcListener.Lib/Const.h"

int main() 
{
	HANDLE hFile = CreateFile(
		L"\\\\.\\ProcessesLifeCycleWatcher", 
		GENERIC_READ, 
		0, 
		nullptr, 
		OPEN_EXISTING, 
	...
	{
		std::cout << "[ERROR]: drive is not running\n";
		std::cin.get();
		return 1;
	}
	...
	while (true) 
	{
		...
		{
			std::cout << "[INFO]: opening process id: " << eventInfo.processId << '\n';
		...
		{
			std::cout << "[INFO]: terminating process id: " << eventInfo.processId << '\n';
