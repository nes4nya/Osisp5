...
extern "C" NTSTATUS DriverEntry(_In_ PDRIVER_OBJECT  DriverObject, _In_ PUNICODE_STRING RegistryPath) 
{
	...
	{
		KdPrint(("[ERROR]: Cannot create device"));
		return status;
	}
	...
	if (!NT_SUCCESS(status)) 
	{
		KdPrint(("[ERROR]: Cannot create link"));
	...
	if (!NT_SUCCESS(status)) 
	{
		KdPrint(("[ERROR]: Cannot register callback"));
...
void OnProcessNotify(PEPROCESS Process, HANDLE ProcessId, PPS_CREATE_NOTIFY_INFO CreateInfo) 
{
	...
	if (CreateInfo != NULL) 
	{
		...
		KdPrint(("[INFO]: Start event enqueued"));
	}
	else 
	{
		...
		ExReleaseFastMutex(&g_Globals.mutex);
		KdPrint(("[INFO]: Exit event enqueued"));
	}
}
